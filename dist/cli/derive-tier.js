...
if (!tierMap) {
    ...
    if (!found) {
        process.stderr.write(`derive-tier: no tier-map found under ${req.repo} (.devloop/ , tools/ , or root)\n`);
        process.exit(2);
    }
    tierMapPath = found;
    tierMap = JSON.parse(readFileSync(found, "utf8"));
}
process.stdout.write(JSON.stringify({ tier: deriveTier(req.touched, tierMap), tierMapPath }) + "\n");
