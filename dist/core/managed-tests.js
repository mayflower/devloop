 ...
 */
export function parseManagedTestGlobs(raw) {
    ...
    const globs = Array.isArray(parsed)
        ? parsed
        : typeof parsed === "object" && parsed !== null && Array.isArray(parsed.globs)
            ? (parsed.globs)
            : null;
    if (globs === null)
        return null; // wrong shape -> fail closed
    if (!globs.every((g) => typeof g === "string"))
        return null; // non-string entry -> fail closed
    return globs;
}
/** Is this repo-relative test path devloop-managed (i.e. does the unskip seam apply to it)? */
export function isManagedTestPath(path, globs) {
