...
const MANAGED_TESTS_SKELETON = JSON.stringify({
    _comment: "Globs of the DEVLOOP-MANAGED test files (written by spec-to-tests, only un-skippable by implement). " +
...
export function initRepo(targetRepo, ciTemplate, opts = {}) {
    ...
    result.notes.push(".devloop/managed-tests.json scopes the unskip seam (§11). The skeleton (`**`) binds EVERY " +
    ...
    if (opts.autoMergeCaller) {
        ...
        if (existsSync(join(targetRepo, rel))) {
            ...
            result.notes.push(`${rel} scaffolded (auto-merge, Variant B) — a HUMAN must push it (a bot lacks the ` +
