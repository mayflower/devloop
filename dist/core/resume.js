...
export function rollupStatus(entries) {
    if (entries.length === 0)
        return "pending";
    if (entries.some((e) => e.status && e.status !== "COMPLETED"))
        return "pending";
    if (entries.some((e) => !PASSING.has(e.conclusion ?? "")))
        return "red";
    return "green";
}
export function reconstructState(facts) {
    const out = { phase: "implemented", humanApprovals: {}, done: false };
    if (facts.reviewDecision === "changes-requested")
        out.reviewDecision = "changes-requested";
    if (isSpecBranch(facts.headBranch)) {
        if (facts.merged) {
            ...
            if (facts.reviewDecision === "approved")
                out.humanApprovals["spec-review"] = true;
        }
        return out;
    }
    // Implementation PR.
