...
export function evaluateHook(input, repo) {
    ...
    if (existsSync(join(repo, ".devloop", "t3-merge.approved")))
        return { block: false };
    return {
        block: true,
        reason: "devloop (anchor a): merge blocked — no local t3-merge approval token in .devloop/. " +
