...
2. **Tier NICHT selbst wählen.** Leite es deterministisch aus den berührten Pfaden gegen die Tier-Map des Ziel-Repos ab:
   ```
   echo '{"touched":[<pfade>],"tierMap":<inhalt von .devloop/tier-map.json>}' \
     | node "${CLAUDE_PLUGIN_ROOT}"/dist/cli/derive-tier.js
   ```
   Schreibe das Ergebnis als `Tier:`-Feld in die `spec.md`.
