...
const looksLikeOwner = (t: string): boolean => t.startsWith("@") || t.includes("@");
...
export function codeownersCovers(samplePath: string, pattern: string): boolean {
  if (pattern === "*" || pattern === "/*") return true;
  const anchored = pattern.startsWith("/");
  let body = pattern.replace(/^\//, "");
  if (body.endsWith("/")) body += "**"; // directory -> everything under it
...
function representativePaths(glob: string): string[] {
  const canonical = glob
    ...
    .replace(/^\//, "");
  const out = [canonical];
  if (glob.startsWith("**/")) out.push(canonical.replace(/^d\//, ""));
  return out;
}

export function findUncoveredTierGlobs(t2t3Globs: string[], ownedPatterns: string[]): string[] {
