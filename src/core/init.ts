...
import { tierGlobs, type TierMapInput } from "./tier.js";
...
function tierGlobToCodeownersPath(glob: string): string {
  if (glob.startsWith("**/")) {
...
export function initRepo(
  ...
  const writeIfAbsent = (relPath: string, content: string): void => {
    if (existsSync(join(targetRepo, relPath))) result.skipped.push(relPath);
    else write(relPath, content);
  };
  ...
  if (existsSync(join(targetRepo, workflow)) && !opts.force) {
    ...
    result.notes.push(
      `⚠️ ${workflow} exists and was NOT updated — re-run with --force to migrate it, then a ` +
  ...
  if (existingTierMap) {
    ...
    result.notes.push(
      `tier-map already present (${existingTierMap.replace(targetRepo + "/", "")}) — not writing ` +
  ...
  if (tierMapPath) {
    const t2t3 = tierGlobs(
      JSON.parse(readFileSync(tierMapPath, "utf8")) as TierMapInput,
      ["T2", "T3"],
    );
    const coRel = ["CODEOWNERS", ".github/CODEOWNERS", "docs/CODEOWNERS"].find((r) =>
      existsSync(join(targetRepo, r)),
    );
    if (!coRel) {
      const owners = [...new Set(t2t3.map(tierGlobToCodeownersPath))].map((p) => `${p} @OWNER`);
      write(
        ".github/CODEOWNERS",
        "# devloop: T2/T3 tier paths require a human CODEOWNER review (the §9 merge gate).\n" +
      ...
      const uncovered = findUncoveredTierGlobs(
        t2t3,
        parseCodeowners(readFileSync(join(targetRepo, coRel), "utf8")),
      );
      if (uncovered.length > 0) {
        result.notes.push(
          `CODEOWNERS (${coRel}) does not cover these T2/T3 tier paths — add owner lines, else such ` +
