...
import { planPrune, type BranchInfo } from "../core/prune.js";
...
function defaultBranch(): string {
  ...
  for (const candidate of ["main", "master"]) {
    if (gitSafe(["rev-parse", "--verify", candidate])) return candidate;
  }
  return "main";
}
...
const branches: BranchInfo[] = all.map((name) => ({
  name,
...
if (apply) {
  for (const d of plan.delete) {
    if (d.worktreePath) gitSafe(["worktree", "remove", d.worktreePath]);
    try {
...
process.stdout.write(
  JSON.stringify({ base, current, plan, applied: apply, results }, null, 2) + "\n",
);
